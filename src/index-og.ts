//first thing taht we need is the action types written as constants
...
// 2) define actions and action type
...
const todosReducer = (
    ...
    type SelectedTodoActionTyeps = SelectTodoActionType;
    const selectedTodoReducer = (
        state: string | null = null, 
        action: SelectedTodoActionTyeps
    ) =>{
        switch (action.type) {
            case SELECT_TODO: {
                const {payload} = action; 
